...
def test_assemble_finding_records_real_signup_finding(real_conn):
    # data/recon.db currently has six real, human-verified findings:
    ...
    # finding_id=5 (/signup, manual_payload, stacked-queries file write --
    # tests/reading_writing_files_writeup.md), and finding_id=6 (/signup,
    # manual_payload, command execution -- tests/command_execution_writeup.md).
...
def test_only_confirmed_verified_findings_are_included(real_conn):
    ...
    real_conn.commit()
    records = assemble_finding_records(real_conn)
    # The four seeded real findings (/signup, /find-user, /forgot, /profile/{id}) --
    # the needs_review and unverified-confirmed rows just inserted must not appear.
    assert {r["endpoint"] for r in records} == {"/signup", "/find-user", "/forgot", "/profile/{id}"}
...
def test_export_report_sqlmap_json_is_valid_json(real_conn, tmp_path):
    ...
    assert payload["schema_version"] == "sqlmap_candidate_v1"
    # 3 reactive signupPOST candidates (name/username/email) x 3 findings that
    # all reference source_trace_id=4 (finding_id=1, 5, 6) + 1 null-fallback
